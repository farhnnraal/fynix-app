...
import { useFormState } from "react-dom";
  ...
  return (
    <div className="w-full h-dvh flex flex-col items-center max-w-[390px] mx-auto bg-neutral-50 min-h-screen pb-[161px] relative">
      ...
      {/* <div className="absolute bottom-0 left-0 right-0 p-6 bg-white rounded-t-3xl z-40 shadow-lg border-t border-neutral-100">
        {isPassed ? (
          nextLevelTopic ? (
            <button
              type="button"
              ...
              className="w-full px-4 py-3 text-btn-lg bg-primary-500 flex items-center justify-center text-white rounded-lg font-semibold hover:bg-primary-600 transition-all"
            >
              Next Level: {nextLevelTopic.title} ({nextLevelTopic.difficulty})
            </button>
          ) : (
            <button
              type="button"
              onClick={() => router.push('/dashboard')}
              className="w-full px-4 py-3 text-btn-lg bg-success-600 flex items-center justify-center text-white rounded-lg font-semibold"
            >
              Back to Dashboard (All Materials Completed!)
            </button>
          )
        ) : (
          <button
            ...
            className="w-full px-4 py-3 text-btn-lg bg-destructive-500 hover:bg-destructive-600 text-white flex items-center justify-center rounded-lg font-semibold transition-all"
          >
            Retry Quiz (Score {finalScore}% - Need 75%)
          </button>
        )}
      </div> */}
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-white rounded-t-3xl z-40">
        ...
        {isPassed && nextLevelTopic && (
          <button
            ...
            onClick={() => {
              const targetId =
                nextLevelTopic.topic_id ||
                nextLevelTopic.custom_topic_id;
